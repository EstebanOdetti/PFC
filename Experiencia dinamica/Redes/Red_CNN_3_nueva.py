...
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.2, random_state=5, shuffle=False
)
...
class CombinedCNN(nn.Module):
    def __init__(self, input_channels, num_rows, output_size):
        ...
        self.front_wheel_conv1 = nn.Conv1d(
...
X_train_tensor_combined_cnn = torch.tensor(X_train, dtype=torch.float32).view(
    -1, input_channels, num_rows
)
y_train_tensor_combined_cnn = torch.tensor(y_train, dtype=torch.float32)
X_test_tensor_combined_cnn = torch.tensor(X_test, dtype=torch.float32).view(
    -1, input_channels, num_rows
)
y_test_tensor_combined_cnn = torch.tensor(y_test, dtype=torch.float32)


train_dataset_combined_cnn = TensorDataset(
    X_train_tensor_combined_cnn, y_train_tensor_combined_cnn
)
test_dataset_combined_cnn = TensorDataset(
    X_test_tensor_combined_cnn, y_test_tensor_combined_cnn
)

train_loader_combined_cnn = DataLoader(
    train_dataset_combined_cnn, batch_size=4, shuffle=True
)
test_loader_combined_cnn = DataLoader(
    test_dataset_combined_cnn, batch_size=4, shuffle=False
)
...
for i, ax in enumerate(axes.flatten()):
    ax.scatter(
        y_test_np[:, i],
        predictions_combined_cnn[:, i],
        label=f"{feature_labels[i]} (Predictions)",
        color="blue",
    )
    ax.scatter(
        y_test_np[:, i],
        y_test_np[:, i],
        label=f"{feature_labels[i]} (Ground Truth)",
        color="red",
        marker="x",
    )
